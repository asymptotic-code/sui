...
description: Create a formal verification spec plan for the is_preactive_spec function in staking pool.
cwd: /Users/cos/asymptotic/agent/clients/mysten/sui/crates/sui-framework/packages/sui-system
started_at: 2026-06-15T23:31:38.284145
foxy_commit: 38346c7c25594d3c381dff95b53fe33dba150411
prompt_part_hashes: {"base": "5fdb6c5e65d5df8a", "core": "35fc153c53e2c232", "file_ops": "b76d200c47b2271e", "function_knowledge": "ec5c60d9b1e6f113", "project_env": "21a3de2d42771978", "spec_writeup": "61585c0ab1a10352", "_global": "6873a7eee52d7fef079862ddb168cf939091e6530a415a8684d4036716326242"}
---
...
Produce a spec plan for `staking_pool_specs::is_preactive_spec` — a YAML document capturing what should be formally verified about this function. Analyze the function source, its callees, dependency graph, and role in the protocol. The analysis guidelines and output schema are in your system prompt.
...
## Target Function: `staking_pool_specs::is_preactive_spec`
...
Produce the spec plan for `staking_pool_specs::is_preactive_spec`. Return it via `final_result(spec_plan)` as a Python dict matching the YAML schema in your system prompt.

## Assistant

### Code `toolu_01SgtJJQCLqb2gA2ctRR9CJv`
...
> 12576 in, 123 out, 15801 cache read

### Result `toolu_01SgtJJQCLqb2gA2ctRR9CJv`
...
The `all_accesses` lookup failed due to a compilation error in the broader package. However, the function source is simple and directly inspectable from the module provided. Let me analyze it directly.

### Code `toolu_01QuLdUMw9AWJX4pWvVdtJJX`
...
spec_plan = {
    ...
    "complexity": "low",
    "summary": "Returns true if and only if the staking pool has never been activated — i.e., pool.activation_epoch is Option::None, indicating the pool is in the pre-active (validator-candidate) state.",
    "role": "Read-only predicate over StakingPool lifecycle state. Called internally by request_withdraw_stake (to decide between immediate vs queued withdrawal paths), by activate_staking_pool (as a precondition guard against double-activation via EPoolAlreadyActive), by convert_to_fungible_staked_sui (EPoolPreactiveOrInactive guard), and by is_preactive_at_epoch. In the broader protocol it gates whether a pool can accept normal staking operations or must fall back to special pre-active handling.",
    "aborts": [],
    "requires": [],
    "ensures": [
        {
            "condition": "result == pool.activation_epoch.is_none()",
            "reason": "The return value is exactly the Option::is_none check on activation_epoch — true when the pool has never been activated, false when activation_epoch has been filled."
        },
        {
        ...
        },
        {
            ...
            "reason": "Negative result means the pool has been activated (active or inactive — both have Some activation_epoch)."
        }
    ],
    "observations": [
        "The predicate does not distinguish between active and inactive pools — both have activation_epoch set to Some, so is_preactive returns false for both. The deactivation_epoch field is not consulted.",
        "A pool created by staking_pool::new() always starts preactive (activation_epoch initialized to option::none()); this predicate validates that initial condition.",
        "The function does not abort under any circumstances — it is a pure boolean projection of a single Option field."
    ]
}
...
> 110139 in, 618 out, 15801 cache read, 12699 cache create

### Result `toolu_01QuLdUMw9AWJX4pWvVdtJJX`

<!-- {"executed":true,"return_value":{"function":"staking_pool_specs::is_preactive_spec","complexity":"low","summary":"Returns true if and only if the staking pool has never been activated \u2014 i.e., pool.activation_epoch is Option::None, indicating the pool is in the pre-active (validator-candidate) state.","role":"Read-only predicate over StakingPool lifecycle state. Called internally by request_withdraw_stake (to decide between immediate vs queued withdrawal paths), by activate_staking_pool (as a precondition guard against double-activation via EPoolAlreadyActive), by convert_to_fungible_staked_sui (EPoolPreactiveOrInactive guard), and by is_preactive_at_epoch. In the broader protocol it gates whether a pool can accept normal staking operations or must fall back to special pre-active handling.","aborts":[],"requires":[],"ensures":[{"condition":"result == pool.activation_epoch.is_none()","reason":"The return value is exactly the Option::is_none check on activation_epoch \u2014 true when the pool has never been activated, false when activation_epoch has been filled."},{"condition":"result == true implies pool.activation_epoch == option::none()","reason":"Positive result means no activation epoch has been recorded."},{"condition":"result == false implies pool.activation_epoch.is_some()","reason":"Negative result means the pool has been activated (active or inactive \u2014 both have Some activation_epoch)."}],"observations":["The predicate does not distinguish between active and inactive pools \u2014 both have activation_epoch set to Some, so is_preactive returns false for both. The deactivation_epoch field is not consulted.","A pool created by staking_pool::new() always starts preactive (activation_epoch initialized to option::none()); this predicate validates that initial condition.","The function does not abort under any circumstances \u2014 it is a pure boolean projection of a single Option field."]}} -->
```
✓ ok
```
...
| model | claude-sonnet-4-6 |
| prompt | 6873a7eee52d7fef079862ddb168cf939091e6530a415a8684d4036716326242 |
| turns | 2 |
| tool_calls | 2 |
| time | 20.11s |
| ended_at | 2026-06-15T23:31:58.393505 |
| cost | $0.4364 |
| tokens | 122715 in / 741 out |
| cache | 31602 read / 12699 create |


## Knowledge Output

- field: spec/writeup/staking_pool_specs::is_preactive_spec
- data: /Users/cos/asymptotic/agent/clients/mysten/sui/crates/sui-framework/packages/sui-system/specs/sources/staking_pool_specs.foxy/is_preactive_spec/spec.writeup.yaml
- meta: /Users/cos/asymptotic/agent/clients/mysten/sui/crates/sui-framework/packages/sui-system/specs/sources/staking_pool_specs.foxy/is_preactive_spec/writeup.meta.yaml
