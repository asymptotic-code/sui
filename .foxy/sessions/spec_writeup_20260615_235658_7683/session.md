...
description: Creating formal verification spec plan for staking pool split function with dependency analysis.
cwd: /Users/cos/asymptotic/agent/clients/mysten/sui
started_at: 2026-06-15T23:56:58.768588
foxy_commit: 38346c7c25594d3c381dff95b53fe33dba150411
prompt_part_hashes: {"base": "5fdb6c5e65d5df8a", "core": "35fc153c53e2c232", "file_ops": "b76d200c47b2271e", "function_knowledge": "ec5c60d9b1e6f113", "project_env": "21a3de2d42771978", "spec_writeup": "61585c0ab1a10352", "_global": "484817d91de0e441b6a92da397946799e8ef56028f57b3736d84e061cf513fdb"}
---
...
Produce a spec plan for `staking_pool_specs::split_spec` — a YAML document capturing what should be formally verified about this function. Analyze the function source, its callees, dependency graph, and role in the protocol. The analysis guidelines and output schema are in your system prompt.
...
## Target Function: `staking_pool_specs::split_spec`
...
Produce the spec plan for `staking_pool_specs::split_spec`. Return it via `final_result(spec_plan)` as a Python dict matching the YAML schema in your system prompt.

## Assistant

### Code `toolu_0185Tk9wsipSvAUhefgy3u8G`
...
. If stdlib/external with no side effects, add to EXTERNAL_CALLEES_WHITELIST.
```
...
The compilation fails (a known tooling limitation for this project), but I have everything needed from the source directly. The function is self-contained with 0 transitive deps — I can derive the full spec plan directly from the source analysis.

### Code `toolu_01RUK3U7A9bQ35reXCGunPtd`
...
spec_plan = {
    ...
    "complexity": "low",
    "summary": (
        "Splits a StakedSui object into two parts: a new StakedSui with principal equal to "
        "split_amount, and the original object retaining the remaining principal. All metadata "
        "(pool_id, stake_activation_epoch) is copied to the new object unchanged. Both resulting "
        "parts must satisfy the minimum staking threshold of 1 SUI (1_000_000_000 MIST)."
    ),
    "role": (
        "User-facing utility on StakedSui objects, callable directly via staking_pool::split "
        "or through split_staked_sui (public entry). Enables holders to partition a staked "
        "position without affecting pool accounting — no interaction with the StakingPool state "
        "itself, purely a self-custody object operation."
    ),
    "aborts": [
        {
            "condition": "split_amount > self.principal.value()",
            "reason": "Cannot split more than the available principal (EInsufficientSuiTokenBalance = 3)"
        },
        {
            "condition": "self.principal.value() - split_amount < MIN_STAKING_THRESHOLD",
            "reason": "Remaining principal would drop below 1 SUI threshold (EStakedSuiBelowThreshold = 18)"
        },
        {
            "condition": "split_amount < MIN_STAKING_THRESHOLD",
            "reason": "Split-off amount would be below 1 SUI threshold (EStakedSuiBelowThreshold = 18)"
        },
    ],
    "requires": [
    ...
    "observations": [
        "No StakingPool state is touched; this is a pure object-split with no accounting side-effects.",
        "Both the minimum threshold checks use the same error code (EStakedSuiBelowThreshold), so "
        "distinguishing which of the two postcondition violations triggered requires inspecting the amounts.",
...
| model | claude-sonnet-4-6 |
| prompt | 484817d91de0e441b6a92da397946799e8ef56028f57b3736d84e061cf513fdb |
| turns | 2 |
| tool_calls | 2 |
| time | 37.34s |
| ended_at | 2026-06-15T23:57:36.105274 |
| cost | $0.4978 |
| tokens | 123058 in / 1061 out |
| cache | 15801 read / 28798 create |
| sub-agent tokens | 432 in / 191 out |
| total tokens | 123490 in / 1252 out |
| sub-agent cache | 0 read / 8007 create |
| total cache | 15801 read / 36805 create |


## Knowledge Output

- field: spec/writeup/staking_pool_specs::split_spec
- data: /Users/cos/asymptotic/agent/clients/mysten/sui/crates/sui-framework/packages/sui-system/specs/sources/staking_pool_specs.foxy/split_spec/spec.writeup.yaml
- meta: /Users/cos/asymptotic/agent/clients/mysten/sui/crates/sui-framework/packages/sui-system/specs/sources/staking_pool_specs.foxy/split_spec/writeup.meta.yaml
