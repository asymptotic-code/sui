...
Review the spec for `staking_pool_specs::split_fungible_staked_sui_spec`. Compare the actual spec against the writeup (the verification plan), using the function source, callees, and protocol context to assess coverage.
...
This is what should be verified about `staking_pool_specs::split_fungible_staked_sui_spec`:

```yaml
function: staking_pool::split_fungible_staked_sui
complexity: low
summary: Splits a FungibleStakedSui object by deducting split_amount from the original's
  value and constructing a new FungibleStakedSui object with that amount, both sharing
  the same pool_id. The total pool-token supply represented by the pair is conserved.
role: 'Utility function on FungibleStakedSui (aliased as FungibleStakedSui.split).
  Enables partial transfer or partial redemption of fungible staked positions: a holder
  can split their fungible stake into two independently owned objects without interacting
  with the StakingPool itself. Called directly by users; not gated by any entry-point
  in sui_system.'
aborts:
- condition: split_amount > fungible_staked_sui.value
  reason: Requested split amount exceeds available pool-token balance (EInsufficientPoolTokenBalance
    = 0)
requires:
...
observations:
- No minimum split size is enforced for FungibleStakedSui (unlike StakedSui.split
  which enforces MIN_STAKING_THRESHOLD on both halves). A split of 0 is permitted
  by the assert guard (0 <= value is always true), producing an empty FungibleStakedSui.
- The pool_id is copied by value, so both objects refer to the same staking pool —
  pool membership is preserved across splits.
- This function does not touch StakingPool state; no pending withdraw or supply accounting
  is updated. The pool's FungibleStakedSuiData.total_supply is unchanged.
```
...
Statement counts: 1 asserts, 0 requires, 0 ensures
...
Review the spec for `staking_pool_specs::split_fungible_staked_sui_spec`. Produce a markdown review via `final_result(format_review_md(function, verdict, issues, analysis, strengths))`.

## Assistant

I'll review this spec by analyzing the function's accesses and comparing the spec against the writeup.

### Code `toolu_01Rniu3kpwHCbWLwNG8dJG4V`
...
> 17595 in, 187 out, 64254 cache read

### Result `toolu_01Rniu3kpwHCbWLwNG8dJG4V`
...
The `all_accesses` lookup failed because the project doesn't compile standalone (it's the Sui framework `sui-system` package which needs framework deps). I'll proceed with manual analysis since I have the full function source, module, and writeup.

### Code `toolu_016jyAqnwqiywTYHtMMzuU63`

```python
from foxy.skills.spec_review.api import format_review_md

function = "staking_pool_specs::split_fungible_staked_sui_spec"

analysis = """The target `split_fungible_staked_sui` is a simple, self-contained utility: it asserts `split_amount <= fungible_staked_sui.value`, deducts `split_amount` from the original's `value`, and packs a new `FungibleStakedSui { id: object::new(ctx), pool_id, value: split_amount }`. It has no callees, touches no `StakingPool` state, and the only abort source is the explicit `assert!(split_amount <= value, EInsufficientPoolTokenBalance)`.
...
issues = [
    ...
    {
        "description": "No ensures on pool membership preservation: result.pool_id == fungible_staked_sui.pool_id is unverified. The split is meant to keep both halves bound to the same pool; nothing confirms the new object copies the correct pool_id.",
        "severity": "medium",
        "confidence": "high",
    },
]
...
| model | claude-opus-4-8 |
| prompt | e8ec3899689ba985adf028b38cd94275e16f99dedce20367f5abd30d813b6249 |
| turns | 2 |
| tool_calls | 2 |
| time | 31.93s |
| ended_at | 2026-06-16T00:13:23.187197 |
| cost | $1.0608 |
| tokens | 167107 in / 1994 out |
| cache | 128508 read / 17782 create |


## Knowledge Output

- field: spec/review/staking_pool_specs::split_fungible_staked_sui_spec
- data: /Users/cos/asymptotic/agent/clients/mysten/sui/crates/sui-framework/packages/sui-system/specs/sources/staking_pool_specs.foxy/split_fungible_staked_sui_spec/spec.review.md
- meta: /Users/cos/asymptotic/agent/clients/mysten/sui/crates/sui-framework/packages/sui-system/specs/sources/staking_pool_specs.foxy/split_fungible_staked_sui_spec/review.meta.yaml
