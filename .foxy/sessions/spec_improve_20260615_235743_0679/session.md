...
Improve the spec `staking_pool_specs::split_spec` targeting `staking_pool::split`. Current state: stub. Add abort coverage (asserts) and remove ignore_abort.

## Assistant

I'll start by loading the spec_improve skill and gathering context about the spec and its target.

### Code `toolu_01XqvhPLzyDu1gGytPbgJDAW`

```python
load_skill('spec_improve')
from foxy.skills.spec_knowledge.api import get_knowledge as get_spec_knowledge
from foxy.skills.project_env.api import get_project_root, get_packages
...
> 137 in, 199 out, 105847 cache read

### Result `toolu_01XqvhPLzyDu1gGytPbgJDAW`

<!-- {"executed":true,"stdout":"/var/folders/n2/rx0nqjvn0px8cv2gw3x6f1ww0000gn/T/foxy_stdout_niaeyuyl.txt","to_foxy_logs":"project=/Users/cos/asymptotic/agent/clients/mysten/sui\npackages=['/Users/cos/asymptotic/agent/clients/mysten/sui/crates/sui-framework/packages/sui-system', '/Users/cos/asymptotic/agent/clients/mysten/sui/crates/sui-framework/packages/sui-system/specs']"} -->
```
...
### Code `toolu_01WyNBq2RC79L9Udn8DXpLGd`

```python
k = get_spec_knowledge(project)
spec = k.item("staking_pool_specs::split_spec")
...
### Code `toolu_01VeJNv8PNb7hvmpZJPQcuZH`
...
for c in spec.callees:
    foxy_inspect(f"--- {c['name']} ---")
    foxy_inspect(c.get('source_code', c.get('body', '')))
```

> 365 in, 181 out, 106183 cache read, 378 cache create

### Result `toolu_01VeJNv8PNb7hvmpZJPQcuZH`
...
### Code `toolu_0145xcCyrwcja5nV1eWMz2qq`

```python
foxy_inspect(f"target type: {type(spec.target)}")
foxy_inspect(f"target keys: {list(spec.target.keys()) if isinstance(spec.target, dict) else 'n/a'}")
```

> 31 in, 120 out, 106561 cache read, 546 cache create

### Result `toolu_0145xcCyrwcja5nV1eWMz2qq`

<!-- {"executed":true,"stdout":"/var/folders/n2/rx0nqjvn0px8cv2gw3x6f1ww0000gn/T/foxy_stdout_ad8b1q5l.txt","to_foxy_logs":"target type: <class 'dict'>\ntarget keys: ['qualified_name', 'module', 'function_name', 'file_path', 'line_start', 'line_end', 'visibility', 'is_entry', 'function_type', 'attributes', 'source']"} -->
```
...
Now let me look at the callees, particularly `principal.split` (balance::split) and the constants.

### Code `toolu_01Xk2VXKSDgGNWXoBBR5jps9`

```python
foxy_inspect("=== CALLEES ===")
for c in spec.callees:
    foxy_inspect(f"--- {c.get('name', c)} ---")
    body = c.get('source') or c.get('body') or c.get('source_code') or ''
    foxy_inspect(body[:600] if body else '(no body / native)')
```

> 595 in, 188 out, 107258 cache read, 323 cache create

### Result `toolu_01Xk2VXKSDgGNWXoBBR5jps9`

<!-- {"executed":true,"stdout":"/var/folders/n2/rx0nqjvn0px8cv2gw3x6f1ww0000gn/T/foxy_stdout_l9mqco1r.txt","to_foxy_logs":"=== CALLEES ==="} -->
```
=== CALLEES ===
```
...
### Code `toolu_01EtYnyBwQyq1MyqU72UUcUk`

```python
foxy_inspect(f"callees type: {type(spec.callees)}, len: {len(spec.callees) if hasattr(spec.callees, '__len__') else 'n/a'}")
foxy_inspect(spec.callees)
```

> 105 in, 124 out, 107581 cache read, 783 cache create

### Result `toolu_01EtYnyBwQyq1MyqU72UUcUk`

<!-- {"executed":true,"stdout":"/var/folders/n2/rx0nqjvn0px8cv2gw3x6f1ww0000gn/T/foxy_stdout_lrtp2ui3.txt","to_foxy_logs":"callees type: <class 'list'>, len: 0\n[]"} -->
```
...
There's already an existing example at lines 187-188 in the spec file. The constant `MIN_STAKING_THRESHOLD` is already defined in the spec module. Let me look at the spec file around the split_spec and an existing similar spec for the pattern.

### Code `toolu_01ChryrASp5fe5BmUTuU5BvZ`
...
The `split_spec` itself isn't physically in this file yet (it was scaffolded by the collection). Let me check `split_staked_sui` which wraps `split` — that gives me the exact abort coverage pattern. The three asserts there are exactly what `split` aborts on. Let me look at the source of `split_staked_sui` to confirm.

### Code `toolu_01SSDS4NHbTb2u7GB4oJVgih`

```python
foxy_inspect(grep("public fun split_staked_sui\\|public fun staked_sui_amount\\|public fun pool_id\\|public fun stake_activation_epoch", include="staking_pool.move"))
```

> 801 in, 230 out, 110003 cache read, 865 cache create

### Result `toolu_01SSDS4NHbTb2u7GB4oJVgih`

<!-- {"executed":true,"stdout":"/var/folders/n2/rx0nqjvn0px8cv2gw3x6f1ww0000gn/T/foxy_stdout_icszpgue.txt"} -->
```
✓ ok
```
...
### Code `toolu_0172PXHsKjwpEVE1XQC3KJxo`

```python
foxy_inspect(grep("fun split_staked_sui\\|fun staked_sui_amount", include="staking_pool.move"))
```

> 80 in, 92 out, 110868 cache read, 1031 cache create

### Result `toolu_0172PXHsKjwpEVE1XQC3KJxo`

<!-- {"executed":true,"stdout":"/var/folders/n2/rx0nqjvn0px8cv2gw3x6f1ww0000gn/T/foxy_stdout_lid2cgbj.txt"} -->
```
✓ ok
```
...
### Code `toolu_01LMnfDB3icTRY5MtKUpJ3vY`

```python
src_file = "crates/sui-framework/packages/sui-system/sources/staking_pool.move"
foxy_inspect(grep("split_staked_sui", include="staking_pool.move"))
foxy_inspect(grep("staked_sui_amount", include="staking_pool.move"))
```

> 82 in, 143 out, 111899 cache read, 172 cache create

### Result `toolu_01LMnfDB3icTRY5MtKUpJ3vY`

<!-- {"executed":true,"stdout":"/var/folders/n2/rx0nqjvn0px8cv2gw3x6f1ww0000gn/T/foxy_stdout_hdlvs1gi.txt","to_foxy_logs":"\n"} -->
```
...
grep returns silently — empty here because matches are not found (or output went to stdout). Let me check directly.

### Code `toolu_01CSEzef8pkeUde59XKBJhya`

```python
res = grep("split_staked_sui", path="crates/sui-framework/packages/sui-system/sources/staking_pool.move")
foxy_inspect(repr(res))
res2 = grep("staked_sui_amount", path="crates/sui-framework/packages/sui-system/sources/staking_pool.move")
foxy_inspect(repr(res2))
```

> 99 in, 199 out, 112071 cache read, 225 cache create

### Result `toolu_01CSEzef8pkeUde59XKBJhya`
...
### Code `toolu_014hJjvFYXAEgbd6FhfX9jpw`

```python
res = grep("fun split_staked_sui|fun staked_sui_amount|fun pool_id|fun stake_activation_epoch", include="staking_pool.move")
foxy_inspect(repr(res))
```

> 98 in, 116 out, 112296 cache read, 298 cache create

### Result `toolu_014hJjvFYXAEgbd6FhfX9jpw`

<!-- {"executed":true,"stdout":"/var/folders/n2/rx0nqjvn0px8cv2gw3x6f1ww0000gn/T/foxy_stdout_h5f850d8.txt","to_foxy_logs":"''"} -->
```
''
```
...
The grep with `|` alternation returns empty. Let me check the source file content directly for these accessor functions.

### Code `toolu_01Dp5eG6ZYMXXDvs8ZR7e7wc`

```python
content = read(src_file)
import re
for fn in ["fun split_staked_sui", "fun staked_sui_amount", "fun pool_id", "fun stake_activation_epoch", "fun split"]:
    for m in re.finditer(re.escape(fn) + r"\b", content):
        line_no = content[:m.start()].count("\n") + 1
        line = content.splitlines()[line_no-1]
        foxy_inspect(f"L{line_no}: {line.strip()}")
```

> 96 in, 243 out, 112594 cache read, 214 cache create

### Result `toolu_01Dp5eG6ZYMXXDvs8ZR7e7wc`

<!-- {"executed":true,"stdout":"/var/folders/n2/rx0nqjvn0px8cv2gw3x6f1ww0000gn/T/foxy_stdout_uk74dzpz.txt","to_foxy_logs":"L563: public use fun split_staked_sui as StakedSui.split_to_sender;\nL568: public entry fun split_staked_sui(stake: &mut StakedSui, split_amount: u64, ctx: &mut TxContext) {\nL480: public use fun staked_sui_amount as StakedSui.amount;\nL483: public fun staked_sui_amount(staked_sui: &StakedSui): u64 { staked_sui.principal.value() }\nL471: public fun pool_id(staked_sui: &StakedSui): ID { staked_sui.pool_id }\nL485: public use fun stake_activation_epoch as StakedSui.activation_epoch;\nL488: public fun stake_activation_epoch(staked_sui: &StakedSui): u64 {\nL547: public fun split(self: &mut StakedSui, split_amount: u64, ctx: &mut TxContext): StakedSui {"} -->
```
...
Now I have everything. The `split` function aborts on exactly three conditions (the three `assert!` calls), and `split_staked_sui` is just a wrapper that calls `split` then transfers. The existing `split_staked_sui_spec` gives me the exact abort coverage pattern.

`split` differs in that it **returns** the new `StakedSui` (rather than transferring). The abort coverage is identical. Note: `object::new(ctx)` is the only other potential abort source but is treated as non-aborting by the prover.

Let me write the improved spec. First, locate where `split_spec` lives in the file.

### Code `toolu_015a6mxMAM8afXowKJM2jgY2`

```python
m = re.search(r"split_spec", content) if False else None
spec_content = read(spec_file)
for m in re.finditer(r"split_spec\b", spec_content):
    line_no = spec_content[:m.start()].count("\n") + 1
    foxy_inspect(f"L{line_no}: {spec_content.splitlines()[line_no-1].strip()}")
foxy_inspect(f"total lines: {spec_content.count(chr(10))+1}")
```

> 351 in, 400 out, 113147 cache read, 469 cache create

### Result `toolu_015a6mxMAM8afXowKJM2jgY2`

<!-- {"executed":true,"stdout":"/var/folders/n2/rx0nqjvn0px8cv2gw3x6f1ww0000gn/T/foxy_stdout_3_h65nww.txt","to_foxy_logs":"L157: fun split_spec(\ntotal lines: 222"} -->
```
L157: fun split_spec(
total lines: 222
```
...
Now I'll edit the spec to add abort coverage and remove `ignore_abort`. The three abort conditions, using `to_int()` for the subtraction to avoid underflow concerns in the spec arithmetic:

### Code `toolu_017jdkcgucy22jLAuqvv7wyb`

```python
old = """#[spec(prove, target=staking_pool::split, ignore_abort, no_opaque)]
fun split_spec(
    self: &mut StakedSui,
    split_amount: u64,
    ctx: &mut TxContext,
...
> 569 in, 516 out, 114367 cache read, 183 cache create

### Result `toolu_017jdkcgucy22jLAuqvv7wyb`

<!-- {"executed":true,"stdout":"/var/folders/n2/rx0nqjvn0px8cv2gw3x6f1ww0000gn/T/foxy_stdout_i2nxwsnm.txt","to_foxy_logs":"edited"} -->
```
edited
```
...
Now let me run the prover on this spec. The sibling `split_staked_sui_spec` needed `run_on = b"local"` due to cloud out-of-resources, but `split` itself returns the object (no transfer ghost machinery), so it may be lighter. Let me run on cloud first.

### Code `toolu_01VdSCx7yHud43MhDLX6wD2A`
...
result = await run_sui_prover(
    specs_pkg,
    args=["--functions", "split_spec", "--timeout", "60", "--force-timeout", "--cloud"],
    timeout=240,
    label="prove-split_spec",
)
foxy_inspect(result.get("response", "")[-2500:])
```

> 99 in, 282 out, 114550 cache read, 1085 cache create

### Result `toolu_01VdSCx7yHud43MhDLX6wD2A`
...
### Code `toolu_01PSxL56YbhmmLSTJtLZAF4W`

```python
foxy_inspect(f"result type: {type(result)}, value: {result}")
```

> 43 in, 73 out, 115635 cache read, 381 cache create

### Result `toolu_01PSxL56YbhmmLSTJtLZAF4W`

<!-- {"executed":true,"stdout":"/var/folders/n2/rx0nqjvn0px8cv2gw3x6f1ww0000gn/T/foxy_stdout_qg98r8m7.txt","to_foxy_logs":"result type: <class 'NoneType'>, value: None"} -->
```
...
The fork returned None. Let me check task status — it may have been launched as a fork and I need to retrieve the result.

### Code `toolu_01NW5T2JUH4kTbbdFbJcvCiS`
...
The fork is running (#86/#87). I notice `run_sui_prover` returned None because it likely forked and the parent got None back — there seems to be a fork-depth issue. Let me wait for the running task and check its result.

### Code `toolu_01N1LgwhgZJnBkAhsFHGhtET`

```python
import time
time.sleep(60)
for t in list_tasks():
    if 'split_spec' in t['name'] or t['id'] in (86, 87):
        foxy_inspect(f"#{t['id']} {t['name']} [{t['status']}] {t.get('elapsed',0)}s")
        if t.get('response'):
            foxy_inspect(t['response'][-2000:])
```

> 665 in, 245 out, 116132 cache read, 283 cache create

### Result `toolu_01N1LgwhgZJnBkAhsFHGhtET`

<!-- {"executed":true,"stdout":"/var/folders/n2/rx0nqjvn0px8cv2gw3x6f1ww0000gn/T/foxy_stdout_exouz0oh.txt","to_foxy_logs":"#85 prove-split_spec [error] 70s\n#86 prove-split_spec [done] 70s\n#87 sui-prover: specs [done] 66s"} -->
```
...
### Code `toolu_01VEYBfQW5AjHPPW1hDnLWmf`

```python
r = get_task_result(86)
foxy_inspect(r.get("response", "")[-3000:] if isinstance(r, dict) else str(r))
```

> 148 in, 96 out, 116415 cache read, 910 cache create

### Result `toolu_01VEYBfQW5AjHPPW1hDnLWmf`
...
### Code `toolu_01Jsp8heaRGJ7zj3UEJrkxoe`

```python
foxy_inspect(f"type={type(r)}, value={repr(r)[:500]}")
r87 = get_task_result(87)
foxy_inspect(f"r87 type={type(r87)}")
if isinstance(r87, dict):
    foxy_inspect(r87.get("response","")[-3000:] or str(r87)[:2000])
```

> 40 in, 162 out, 117325 cache read, 244 cache create

### Result `toolu_01Jsp8heaRGJ7zj3UEJrkxoe`

<!-- {"executed":true,"stdout":"/var/folders/n2/rx0nqjvn0px8cv2gw3x6f1ww0000gn/T/foxy_stdout_9t95szik.txt","to_foxy_logs":"type=<class 'dict'>, value={'status': 'success', 'result': {'summary': 'PASS split_spec', 'details': [{'spec': 'split_spec', 'status': 'PASS', 'phases': ['_Check (2s)', '_Assume', '_SpecNoAbortCheck']}], 'totals': {'passed': 1, 'failed': 0, 'timeout': 0}, 'raw_exit_code': 0}, 'response': None, 'stop_reason': 'tool_use', 'turns': 2, 'tokens_used': 909, 'usage': {'input_tokens': 548, 'output_tokens': 361, 'cache_read_input_tokens': 283054, 'cache_creation_input_tokens': 8929}, 'total_usage': {'input_tokens': 548, 'output_to\nr87 type=<class 'dict'>\n{'returncode': 0, 'stdout': '\ud83d\udd04 staking_pool_specs::split_spec_Check\\n\ud83d\udd04 staking_pool_specs::split_spec_Assume\\n\ud83d\udd04 staking_pool_specs::split_spec_SpecNoAbortCheck\\n\u2705 staking_pool_specs::split_spec_SpecNoAbortCheck\\n\u2705 staking_pool_specs::split_spec_Assume\\n\u2705 staking_pool_specs::split_spec_Check (2s)\\nVerification successful\\n'}"} -->
```
...
The spec passes all three phases on the cloud prover:
- `_Check` ✅ (2s)
- `_Assume` ✅
- `_SpecNoAbortCheck` ✅

All abort coverage is verified — the asserts exactly correspond to the function's abort conditions, with no `ignore_abort`. Let me confirm the final spec content.

### Code `toolu_01LvNny7fAuFcKTrWLwkSRaq`

```python
foxy_inspect(read(spec_file, 156, 172))
```

> 570 in, 185 out, 117569 cache read, 202 cache create

### Result `toolu_01LvNny7fAuFcKTrWLwkSRaq`

<!-- {"executed":true,"stdout":"/var/folders/n2/rx0nqjvn0px8cv2gw3x6f1ww0000gn/T/foxy_stdout_4pkdrl9o.txt","to_foxy_logs":"// @VERIFY(\ud83d\udee1\ufe0f/\u2705)\n#[spec(prove, target=staking_pool::split, no_opaque)]\nfun split_spec(\n    self: &mut StakedSui,\n    split_amount: u64,\n    ctx: &mut TxContext,\n): StakedSui {\n    let original_amount = staking_pool::staked_sui_amount(self);\n    asserts(split_amount <= original_amount);\n    asserts(original_amount.to_int().sub(split_amount.to_int()).gte(MIN_STAKING_THRESHOLD.to_int()));\n    asserts(split_amount >= MIN_STAKING_THRESHOLD);\n    staking_pool::split(self, split_amount, ctx)\n}\n\n#[spec(prove, target=staking_pool::split_fungible_staked_sui, ignore_abort)]\nfun split_fungible_staked_sui_spec(\n    fungible_staked_sui: &mut FungibleStakedSui,\n"} -->
```
// @VERIFY(🛡️/✅)
#[spec(prove, target=staking_pool::split, no_opaque)]
fun split_spec(
...
`staking_pool_specs::split_spec` improved from stub → abort coverage complete, passing all three prover phases (`_Check` 2s, `_Assume`, `_SpecNoAbortCheck`) on the cloud.

What changed:
- Removed `ignore_abort`.
- Added the three `asserts` matching `split`'s exact abort conditions:
  - `split_amount <= original_amount` (`EInsufficientSuiTokenBalance`)
  - `remaining_amount >= MIN_STAKING_THRESHOLD`, expressed in `to_int()` arithmetic (`EStakedSuiBelowThreshold`)
  - `split_amount >= MIN_STAKING_THRESHOLD` (`EStakedSuiBelowThreshold`)
- Updated the annotation to `// @VERIFY(🛡️/✅)`.

The `_Assume` pass confirms these asserts are exactly the abort coverage (no over/under-approximation). Want me to push this to the `semantics` stage next — adding `ensures` for the returned `StakedSui` (principal `== split_amount`, same `pool_id`/`stake_activation_epoch`, and `self` reduced by `split_amount`)?

> 438 in, 412 out, 117771 cache read, 755 cache create
...
| model | claude-opus-4-8 |
| prompt | 7308dedb720c36cf986bc02915f246178b151699fb98cfcb7e8ab1b52cd18c31 |
| turns | 28 |
| tool_calls | 27 |
| time | 187.10s |
| ended_at | 2026-06-16T00:00:50.163172 |
| cost | $1.8187 |
| tokens | 8122 in / 5407 out |
| cache | 3127378 read / 12679 create |
| sub-agent tokens | 548 in / 361 out |
| total tokens | 8670 in / 5768 out |
| sub-agent cache | 283054 read / 8929 create |
| total cache | 3410432 read / 21608 create |


## Knowledge Output

- field: spec/improve/staking_pool_specs::split_spec
- data: /Users/cos/asymptotic/agent/clients/mysten/sui/crates/sui-framework/packages/sui-system/specs/sources/staking_pool_specs.foxy/split_spec/spec.improve.yaml
- meta: /Users/cos/asymptotic/agent/clients/mysten/sui/crates/sui-framework/packages/sui-system/specs/sources/staking_pool_specs.foxy/split_spec/improve.meta.yaml
