...
harness: cc
started_at: 2026-06-15T22:49:08.869149
foxy_commit: 38346c7c25594d3c381dff95b53fe33dba150411
prompt_part_hashes: {"base": "5fdb6c5e65d5df8a", "core": "35fc153c53e2c232", "file_ops": "b76d200c47b2271e", "function_knowledge": "ec5c60d9b1e6f113", "project_env": "21a3de2d42771978", "spec_writeup": "61585c0ab1a10352", "_global": "c026fbaf0d33c0e2930e3f9f4826cf5146d4fe887ef1a95a1812aefafe6040b3"}
parent_session: null
effort: max
timeout: 300
token_budget: null
...
Produce a spec plan for `staking_pool_specs::split_staked_sui_spec` — a YAML document capturing what should be formally verified about this function. Analyze the function source, its callees, dependency graph, and role in the protocol. The analysis guidelines and output schema are in your system prompt.
...
## Target Function: `staking_pool_specs::split_staked_sui_spec`
...
Produce the spec plan for `staking_pool_specs::split_staked_sui_spec`. Return it via `final_result(spec_plan)` as a Python dict matching the YAML schema in your system prompt.
## Turn 2 — `ToolSearch`
...
spec_plan = {
    ...
    "complexity": "low",
    "summary": (
        "Entry point that splits a StakedSui into two parts — the original stake has its "
        "principal reduced by split_amount, and a new StakedSui with that principal is "
        "created and transferred to the transaction sender. Both resulting parts must meet "
        "the 1 SUI minimum threshold."
    ),
    "role": (
        "User-facing entry point allowing StakedSui holders to partition their staking "
...
- The three abort conditions come entirely from the inner `split` function — the entry wrapper itself has no extra assertions. The abort for `EInsufficientSuiTokenBalance` (condition 1) is actually redundant with the threshold checks when `split_amount >= MIN_STAKING_THRESHOLD` and the remaining `>= MIN_STAKING_THRESHOLD`, but it must still be stated because the prover sees `split_amount <= original_amount` as a separate guard.
- The `requires` and the first two `aborts` are symmetrically linked: the non-abort preconditions are exactly the negations of the abort conditions.
- All metadata (`pool_id`, `stake_activation_epoch`) is preserved on both the mutated original and the new object — only the `principal` balance moves. This is the primary frame condition of interest.
- The transfer target (`ctx.sender()`) is not formally capturable as an `ensures` on the sender's ownership in the prover, but it is the semantic postcondition of the entry point.
...
| model | claude-sonnet-4-6 |
| effort | max |
| prompt | c026fbaf0d33c0e2930e3f9f4826cf5146d4fe887ef1a95a1812aefafe6040b3 |
| turns | 3 |
| tool_calls | 2 |
| time | 58.49s |
| ended_at | 2026-06-15T22:50:07.381698 |
| tokens | 9 in / 3781 out |
| cache | 59746 read / 119081 create |
| cost | $0.8022 |


## Knowledge Output

- field: spec/writeup/staking_pool_specs::split_staked_sui_spec
- data: /Users/cos/asymptotic/agent/clients/mysten/sui/crates/sui-framework/packages/sui-system/specs/sources/staking_pool_specs.foxy/split_staked_sui_spec/spec.writeup.yaml
- meta: /Users/cos/asymptotic/agent/clients/mysten/sui/crates/sui-framework/packages/sui-system/specs/sources/staking_pool_specs.foxy/split_staked_sui_spec/writeup.meta.yaml
