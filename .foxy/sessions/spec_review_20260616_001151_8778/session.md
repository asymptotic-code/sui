...
harness: foxy
parent_session: null
name: staking-pool-inactivity-spec-review
description: Review and verify the is_inactive_spec specification against its implementation and verification plan.
cwd: /Users/cos/asymptotic/agent/clients/mysten/sui
started_at: 2026-06-16T00:11:51.878017
foxy_commit: 38346c7c25594d3c381dff95b53fe33dba150411
prompt_part_hashes: {"base": "5fdb6c5e65d5df8a", "core": "35fc153c53e2c232", "file_ops": "b76d200c47b2271e", "function_knowledge": "ec5c60d9b1e6f113", "project_env": "21a3de2d42771978", "spec_loop": "26d59a7f8c0f21db", "spec_postcondition": "c9935e5df9cbd57c", "spec_precondition": "74781a107ed639cf", "spec_review": "e31ecea77dacc494", "spec_scenario": "d1ce03efba7186ff", "sui_prover_guide": "9b7aaa77fb185386", "_global": "611790d2e3fa5b313ae93f2c19c10fb6507560727b01877906ad025d64855098"}
---
...
Review the spec for `staking_pool_specs::is_inactive_spec`. Compare the actual spec against the writeup (the verification plan), using the function source, callees, and protocol context to assess coverage.
...
This is what should be verified about `staking_pool_specs::is_inactive_spec`:

```yaml
function: staking_pool_specs::is_inactive_spec
complexity: low
summary: Returns true if the staking pool has been deactivated — i.e., if its deactivation_epoch
  field holds a Some value. A pool transitions to inactive when deactivate_staking_pool
  is called, which sets deactivation_epoch to Some(<epoch>). Pre-active and active
  pools have deactivation_epoch == None and return false.
role: 'Read-only predicate on StakingPool used throughout the protocol to gate stake
  operations: request_add_stake aborts with EDelegationToInactivePool when this returns
  true; request_withdraw_stake and process_pending_stake_withdraw branch on it to
  trigger immediate withdrawal processing for inactive pools; deactivate_staking_pool
  uses it to assert no double-deactivation (EDeactivationOfInactivePool); convert_to_fungible_staked_sui
  uses it to assert the pool is neither preactive nor inactive (EPoolPreactiveOrInactive).'
aborts: []
requires: []
ensures:
- condition: result == pool.deactivation_epoch.is_some()
  reason: The return value is exactly the Option::is_some() test on deactivation_epoch
    — true iff the pool was deactivated at some past epoch.
- condition: 'result == true implies exists e: u64 such that pool.deactivation_epoch
    == Some(e)'
  reason: Equivalently, a true result witnesses a concrete deactivation epoch stored
    in the field.
- condition: result == false implies pool.deactivation_epoch == None
  reason: A false result means the pool is either pre-active (activation_epoch.is_none())
    or active; in both cases deactivation_epoch is None.
- condition: pool is not modified
  reason: The function takes an immutable reference; no state changes occur.
observations:
- 'The three pool lifecycle states are encoded in two Option fields: pre-active =
  activation_epoch.is_none(); active = activation_epoch.is_some() && deactivation_epoch.is_none();
  inactive = deactivation_epoch.is_some(). is_inactive checks only the second field,
  so an inactive pool may also have activation_epoch.is_some().'
- 'Once set, deactivation_epoch is never cleared — deactivate_staking_pool guards
  against double-deactivation, and there is no reactivation path. The predicate is
  therefore monotone: once true it stays true.'
- The spec function should be a transparent wrapper (or inlined alias) of staking_pool::is_inactive
  so the prover can reason about callers that branch on pool.is_inactive().
```
...
Review the spec for `staking_pool_specs::is_inactive_spec`. Produce a markdown review via `final_result(format_review_md(function, verdict, issues, analysis, strengths))`.

## Assistant

I'll review the spec for `staking_pool_specs::is_inactive_spec`. Let me first check the function knowledge for accesses.

### Code `toolu_01S6KyJgTq65Bnz6sx2J1F2e`
...
> 17513 in, 195 out, 64254 cache read

### Result `toolu_01S6KyJgTq65Bnz6sx2J1F2e`
...
The accesses query failed due to a vendored compilation issue, but that doesn't affect this review. `is_inactive` is a trivial pure getter — it reads `pool.deactivation_epoch` and returns `is_some()`. I have everything I need from the source. Let me write the review.

### Code `toolu_01Ez6nnfvt11oVbXhheEADRE`
...
analysis = (
    "`is_inactive` is a trivial read-only getter: `pool.deactivation_epoch.is_some()`. "
    "It takes an immutable `&StakingPool`, performs no mutation, and cannot abort, so there is "
    "nothing to model on the aborts or requires side — the empty `aborts: []` / `requires: []` "
    "in the plan is correct and the spec rightly omits asserts.\n\n"
    "The gap is entirely on the ensures side. The plan lists four postconditions, all of which "
    "reduce to one equation: `result == pool.deactivation_epoch.is_some()` (the other three — "
    "the true/false witnesses and the no-mutation clause — are corollaries; no-mutation is also "
    "guaranteed by the `&` type). The actual spec contains zero `ensures`, so it proves none of "
    "them. The body `staking_pool::is_inactive(pool)` simply forwards the call; under the default "
    "opaque contract the prover havocs the boolean result and learns nothing, so the spec passes "
    "vacuously. This is the classic 'passes the prover but verifies nothing' situation: consistency "
    "with the implementation is satisfied trivially, while correctness (the return value actually "
    "tracks `deactivation_epoch`) is unchecked.\n\n"
    "The fix is one line: add `ensures(result == pool.deactivation_epoch.is_some())` before "
    "returning (exposing the private field via a `#[test_only, ext(pure)]` accessor or relying on "
    "the inlined `$pure` body if `no_opaque` is used). Because the predicate is the linchpin that "
    "many callers branch on, getting this equation into the spec — transparently — is what makes "
    "the surrounding stake/withdraw/deactivate proofs possible."
)
...
| model | claude-opus-4-8 |
| prompt | 611790d2e3fa5b313ae93f2c19c10fb6507560727b01877906ad025d64855098 |
| turns | 2 |
| tool_calls | 2 |
| time | 27.69s |
| ended_at | 2026-06-16T00:12:19.563859 |
| cost | $1.0474 |
| tokens | 166164 in / 1667 out |
| cache | 128508 read / 17708 create |


## Knowledge Output

- field: spec/review/staking_pool_specs::is_inactive_spec
- data: /Users/cos/asymptotic/agent/clients/mysten/sui/crates/sui-framework/packages/sui-system/specs/sources/staking_pool_specs.foxy/is_inactive_spec/spec.review.md
- meta: /Users/cos/asymptotic/agent/clients/mysten/sui/crates/sui-framework/packages/sui-system/specs/sources/staking_pool_specs.foxy/is_inactive_spec/review.meta.yaml
